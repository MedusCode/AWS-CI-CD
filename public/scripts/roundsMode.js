 ...
 * @function formatDate
 * @Desc
 * Given a date string, return a formatted date string in the format
 * MM/DD/YYYY
...
function fillRoundForm(round) {
  GlobalRoundDate.value = formatDate(round.date);
  ...
  GlobalRoundMinutes.value = round.min;
  GlobalRoundSeconds.value = round.sec < 10 ? "0" + round.sec : round.sec;
...
/*************************************************************************
 * @function clearRoundsTable
 * @desc
 * Clear the "Rounds" table of all rows, leaving only an empty first row
 * with a colspan attribute set to 5. This row will be used to display
 * a message indicating that the table is empty.
 * @global GlobalRoundsTable: The table of rounds
 * @global GlobalRoundsTableCaption: The caption for the "Rounds" table
  *************************************************************************/
function clearRoundsTable() {
  const thisRoundBody = GlobalRoundsTable.querySelector("tbody");
  while (thisRoundBody.rows.length > 1) {
    thisRoundBody.deleteRow(1);
  }
  const emptyRow = thisRoundBody.rows[0];
  emptyRow.innerHTML = "<td colspan='5' scope='rowgroup'><i>No rounds added yet</i></td>";
  GlobalRoundsTableCaption.textContent = "Table displaying 0 speedgolf rounds";
}
...
* @param rIndex -- a 0-based integer index into userData.rounds indicating the
*        round to write to the table.
* @global GlobalUserData: The data for the current user
*************************************************************************/
function writeRoundToTable(row, rIndex) {
row.innerHTML = `<td>${formatDate(GlobalUserData.rounds[rIndex].date)}</td>
                 <td>${GlobalUserData.rounds[rIndex].course}</td>
                 <td>${GlobalUserData.rounds[rIndex].SGS} (${GlobalUserData.rounds[rIndex].strokes} in ${GlobalUserData.rounds[rIndex].time})</td>
                 <td><button aria-label='View and Edit Round' onclick='editRound("${GlobalUserData.rounds[rIndex]._id}")'>
                     <span class='fas fa-eye'></span> <span class='fas fa-edit'></span></button></td>
                 <td><button aria-label='Delete Round' onclick='confirmDelete("${GlobalUserData.rounds[rIndex]._id}")'>
...
const roundId = GlobalUserData.rounds[roundIndex]._id;
...
function updateRoundInTable(rowIndex) {
  const thisRound = document.getElementById("r-" + GlobalUserData.rounds[rowIndex]._id);
  writeRoundToTable(thisRound,rowIndex);
}

/*************************************************************************
 * @function updateTempRoundIds
 * @desc
 * When a pending round is saved, it receives a proper MongoDB ObjectId.
 * This function updates the temporary pending round ids in the 
 * userData.rounds array with the corresponding MongoDB ObjectId of the 
 * newly saved round. It then updates the round id in the corresponding
 * row of the "Rounds" table.
 * @param tempIdToId: An object containing key-value pairs where the key
 *       is the temporary id of a round and the value is the MongoDB
 *      ObjectId of the newly saved round.
 * @global GlobalUserData: The data for the current user
 *************************************************************************/
function updateTempRoundIds(tempIdToId) {
  for (let i = 0; i < Object.keys(tempIdToId).length - 1; ++i) {
    for (let j = 0; j < GlobalUserData.rounds.length; ++j) {
      if (GlobalUserData.rounds[j]._id === Object.keys(tempIdToId)[i]) {
        //Update round id in userData.rounds
        GlobalUserData.rounds[j]._id = tempIdToId[Object.keys(tempIdToId)[i]];
        const thisRound = document.getElementById("r-" + Object.keys(tempIdToId)[i]);
        thisRound.id = "r-" + tempIdToId[Object.keys(tempIdToId)[i]];
        writeRoundToTable(thisRound,j); //update row with new id
        break;
      }
    }
  }
...
for (GlobalRoundIndex = 0; GlobalRoundIndex < GlobalUserData.rounds.length; ++GlobalRoundIndex) {
  if (GlobalUserData.rounds[GlobalRoundIndex]._id === roundId) {
    break;
  }
}
...
//Display dialog
  const stateObj = {page: "VIEW_EDIT_ROUND", path: "/rounds/" + roundId};
  transitionToDialog(GlobalRoundsModeDialog,"SpeedScore: Edit Round", stateObj, 
                    GlobalDialogCancelButtons[1], prepEditRoundForm);
}
...
* round, update the "Rounds"table, return the user to 
...
async function logRound() {
  ...
  const newRound = {
    ...
    seconds: (Number(GlobalRoundMinutes.value) * 60) + Number(GlobalRoundSeconds.value),
    notes: GlobalRoundNotes.value,
  };
  //Call API to add round to database
  Spinner.show();
  const responseObj = await api.addRound(newRound);
  Spinner.hide();
  const results = responseObj.results;
  if (results[0].success) {
    //Push new round object to end of rounds array
    GlobalUserData.rounds.push(results[0].data);
    //Reset form to prepare for next visit
    resetLogRoundForm();
    //Add new round to table
    addRoundToTable(GlobalUserData.rounds.length - 1);
  }
  updateTempRoundIds(responseObj.tempIdToId);
  //Display toast, with one line per response
  let toastMsg = "";
  for (let i = 0; i < results.length; ++i) {
    if (i === 0) {
      toastMsg = results[i].message;
    } else {
      toastMsg += "<br/>" + results[i].message;
    }
  }
  Toast.show(toastMsg);
...
async function updateRound() {
  ...
  GlobalUserData.rounds[GlobalRoundIndex].seconds = (Number(GlobalRoundMinutes.value) * 60) + Number(GlobalRoundSeconds.value);
  ...
  //Call API to update round in database
  Spinner.show();
  const responseObj = await api.updateRound(GlobalUserData.rounds[GlobalRoundIndex]);
  Spinner.hide();
  const results = responseObj.results;
  if (results[0].success) { //Round update was successful.
    //Update round
    GlobalUserData.rounds[GlobalRoundIndex] = results[0].data;
    //Reset form to prepare for next visit
    resetLogRoundForm();
    //Update round in table
