 ...
 *************************************************************************/
 function switchMode(newMode) {
    ...
    if (GlobalHistoryLogging) {
        const newPath = "/" + GlobalModeNames[newMode].replace(/_/g, '').toLowerCase();
        const nextState = {page: GlobalModeNames[newMode], path: newPath};
        console.log("Console: In SwitchMode: Pushing state: " + JSON.stringify(nextState) + " and setting URL to: " + nextState.path);
        history.pushState(nextState, "", nextState.path);
    }
}
